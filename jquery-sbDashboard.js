 ...
 * Copyright 2017, S&B Engineers and Constructors, LTD.
 */
...
( function(factory) {"use strict";
		...
		$.widget("mn.sbDashboard", {
			version : "1.0",
			options : {
				dashboardData : []
			},
			_create : function() {
				this.element.addClass("sbDashboard");
				this._createView();
			...
			_createView : function() {
				...
				$("body").append("<div class='sbDashboard-overlay'></div>");

				$(".sbDashboard-overlay").height(docHeight);

				$(".sbDashboard-overlay").hide();
				...
				this.element.sortable({
					handle : ".sbDashboardWidgetHeader",
			...
			_bindEvents : function() {
				...
				this.element.on("click", ".sbDashboardWidgetHeader div.sbDashboard-icon.sbDashboard-circle-plus-icon", function(e) {

					//get the widget List Item Dom
					var widgetListItem = $(e.currentTarget).parents("li:first");
					//get the widget Container
					var widget = $(e.currentTarget).parents(".sbDashboardWidget:first");
					//get the widget Content
					var widgetContainer = widget.find(".sbDashboardWidgetContent");

					var widgetDefinition = self._getWidgetContentForId(widgetListItem.attr("id"), self);

					//toggle the maximize icon into minimize icon
					$(e.currentTarget).toggleClass("sbDashboard-circle-minus-icon");
					//change the tooltip on the maximize/minimize icon buttons
					if ($(e.currentTarget).attr("title") === "Maximize") {
						$(".sbDashboard-overlay").show();
					...
					widget.toggleClass("sbDashboardWidgetContainerMaximized");
					widgetContainer.toggleClass("sbDashboardWidgetContentMaximized ");

					if (widgetDefinition.widgetType === "chart") {
						var chartArea = widgetContainer.find(" div.sbDashboardChart");
				...
				this.element.on("click", ".sbDashboardWidgetHeader div.sbDashboard-icon.sbDashboard-refresh-icon", function(e) {
				...
				this.element.on("click", ".sbDashboardWidgetHeader div.sbDashboard-icon.sbDashboard-circle-remove-icon ", function(e) {
					var widget = $(e.currentTarget).parents("li:first");
					//show hide effect
				...
				//table row click
				this.element.on("click", ".sbDashboardWidgetContent table.sbDashboardTableView tbody tr", function(e) {
			...
			_constructWidget : function(widgetDefinition) {
				...
				var widgetContainer = $("<div/>").addClass("sbDashboardWidget");

				//create a widget header
				var widgetHeader = $("<div/>").addClass("sbDashboardWidgetHeader sbDashboard-clearfix");
				var maximizeButton = $('<div title="Maximize" class="sbDashboard-icon sbDashboard-circle-plus-icon "></span>');

				var deleteButton = $('<div title="Close" class="sbDashboard-icon sbDashboard-circle-remove-icon"></div>');
				...
				if (widgetDefinition.hasOwnProperty("enableRefresh") && widgetDefinition.enableRefresh) {
					var refreshButton = $('<div title="Refresh" class="sbDashboard-icon sbDashboard-refresh-icon "></div>');
					//add refresh button
				...
				var widgetContent = $("<div/>").addClass("sbDashboardWidgetContent");

				if (widgetDefinition.widgetType === 'table') {
					var dataTable = $('<table cellpadding="0" cellspacing="0" border="0" class="display sbDashboardTableView table table-bordered"></table>');
					widgetContent.append(dataTable);
				} else if (widgetDefinition.widgetType === 'chart') {
					var chart = $('<div/>').addClass("sbDashboardChart");
					if (widgetDefinition.getDataBySelection) {
						chart.addClass("sbDashboardChartSelectable");
					} else {
						chart.addClass("sbDashboardChartClickable");
					}
			...
			_refreshRegularWidget : function(widgetDefinition, widget) {
				var isMaximized = widget.find(".sbDashboardWidgetContent").hasClass('sbDashboardWidgetContentMaximized');
				//first remove the content
				widget.find('.sbDashboardWidgetContent').empty().remove();
				//then create the content again
				var widgetContent = $("<div/>").addClass("sbDashboardWidgetContent");
				//if its maximized add the maximized class
				if (isMaximized) {
					widgetContent.addClass('sbDashboardWidgetContentMaximized');
				}
				widgetContent.append(widgetDefinition.widgetContent);
				//then append this to the widget again;
				widget.find(".sbDashboardWidget").append(widgetContent);
			},
			...
			_renderTable : function(widgetDefinition){
				...
				var table
				if(widgetDefinition.widgetType === 'table'){
					table = this.element.find(id + " table.sbDashboardTableView");
			...
			_renderChart : function(widgetDefinition) {
				...
				if (widgetDefinition.widgetType === 'chart') {
					chartArea = this.element.find(id + " div.sbDashboardChart");
